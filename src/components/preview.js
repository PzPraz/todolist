import {
  ...
  append,
} from "../functions/domHelpers.js";
import { Task } from "../functions/Task.js";
import {
  handleTaskEdit,
  handleTaskFormSubmit,
} from "../logic/taskFormHandlers.js";
...
function createGeneralTasks(title) {
  ...
  const projectPreviewLists = createElementWithClasses(
    "div",
    "project-preview-lists"
  );
...
function createProjectPreview(project) {
  ...
  const addProjectTasksButton = createElementWithClasses(
    "button",
    "add-project-tasks-btn"
  );
...
function createProjectTasksForm() {
  ...
  const taskControlButton = createElementWithClasses(
    "div",
    "add-task-control-btn"
  );
  ...
  const cancelTaskButton = createElementWithClasses(
    "button",
    "cancel-task-btn"
  );
...
function createTaskPreview(task) {
  ...
  const moreDetailsButton = createElementWithClasses(
    "button",
    "more-details-btn"
  );
  const removeTaskButton = createElementWithClasses(
    "button",
    "remove-task-button"
  );
...
function createTaskDetailsPopUp() {
  ...
  const isCompleteButton = createElementWithClasses(
    "button",
    "is-task-complete-btn"
  );
  isCompleteButton.addEventListener("click", (e) => {
    const taskElement =
      e.currentTarget.parentElement.firstChild.lastElementChild;
    const taskId = taskElement.getAttribute("data-task-id");
...
function createTaskEditForm() {
  ...
  const cancelEditButton = createElementWithClasses(
    "button",
    "cancel-task-btn"
  );
